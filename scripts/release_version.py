...
has not: at `v0.10.2`, with the `fix(output)!` from #300 merged, its answer is `v1.0.0`. That would
publish a stable-API claim as a side effect of a naming fix.
...
Run as a script it takes the two versions and prints the answer, which is what the workflow uses:
    ...
    python scripts/release_version.py --current v0.10.1 --proposed "$(git-cliff --bumped-version)"
